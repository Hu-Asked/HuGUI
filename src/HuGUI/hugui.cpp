...
namespace hugui {
    ...
    // Helper functions that don't depend on others
    ...
    // Forward declarations for GUI class static methods
    ...
    // Main API functions
    void GUI::add_auton(std::function<void()> auton, 
                       std::string auton_name, AutonType alliance) {
    ...
    void GUI::add_config_toggle(bool* toggleVariable, std::string name) {
        ...
        if (*toggleVariable) {
            ...
            lv_obj_add_style(btn, &toggleConfigButtonStyleFalse, 0); // Fixed: changed from 1 to 0
        }
    }
    ...
    // Main initialization function (should be last)
